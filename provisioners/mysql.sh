...
do
    ...
    then
        ...
        if [[ -f /vmdir/backup/full-backup-latest.sql ]]; then
            echo -e "\n=> Loading database from backup..."
            #mysql -uroot -p$DBPASSWD < /vmdir/backup/full-backup-$(date +\%F).sql
            sudo mysql -uroot -p$DBPASSWD < /vmdir/backup/full-backup-latest.sql
        else
            echo -e "\n=> Initializing new database..."
            sudo mysql -uroot -p$DBPASSWD < /vmdir/backup/database.sql
        fi
    fi
