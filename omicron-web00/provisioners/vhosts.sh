...
if [[ ! -d /mnt/repo/phpmyadmin ]]; then
    if [ -x "$(command -v git)" ]; then
        echo -e " - [phpmyadmin]: local repository not esxist, cloning from remote"
        #git clone https://github.com/phpmyadmin/phpmyadmin /mnt/repo/phpmyadmin >> /vmdir/log/vm-build-$(date +\%F).log 2>&1
    fi
else
    ln -s "/mnt/repo/phpmyadmin" "/var/www/html/phpmyadmin" >> /vmdir/log/vm-build-$(date +\%F).log 2>&1
fi
...
do
    ...
    if [[ -L /var/www/html/$name ]]; then
        echo -e " - [$name]: removing old symlink"
        rm /var/www/html/$name
    fi

    if [[ ! -z "$gitrepo" && ! -d /mnt/repo/$name ]]; then
        echo -e " - [$name]: local repository not esxist, cloning from remote"
        if [ -x "$(command -v git)" ]; then
            git clone https://github.com/tdopierala/$name /mnt/repo/$name >> /vmdir/log/vm-build-$(date +\%F).log 2>&1
        fi
    fi

    if [[ -d /mnt/repo/$path$name ]]; then
        echo -e " - [$name]: creating new symlink for local repository"
        ln -s "/mnt/repo/$path$name" "/var/www/html/"$name >> /vmdir/log/vm-build-$(date +\%F).log 2>&1
    else
        echo -e " - [$name]: !!! symlink could not be created, directory path '/mnt/repo/$path$name' don't exist"
    fi
done
...
do
    ...
    if [[ -d /mnt/repo/$host ]]; then
        ...
        if [[ ! -d /vmdir/configs/vhosts ]]; then
            mkdir -p /vmdir/configs/vhosts
        fi
...
usermod -a -G www-data vagrant >> /vmdir/log/vm-hosts-$(date +\%F).log 2>&1
